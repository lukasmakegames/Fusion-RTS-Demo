...
public class SC_SelectionManager : MonoBehaviour
{
    ...
    public static GameObject[] GetSelectedGameObjects()
    {
        ...
        foreach (int selectedIndex in selectedObjects)
        {
            if (selectedIndex >= 0 && selectedIndex < selectables.Count)
            {
                SC_Selectable selectedObject = selectables[selectedIndex];
                if (selectedObject != null)
                {
                    selectedGameObjects.Add(selectedObject.gameObject);
                }
            }
        }
